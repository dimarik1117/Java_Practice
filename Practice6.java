...
public class Practice6 {
    ...
    public static String fractions(String number) {
        ...
        int gcd = gcd(numerator, denominators[1]);
    ...
    public static int generateNonconsecutive(String str) throws CustomException {
        String operations = "+-/*";
        str = str.replaceAll("\\(", "( ");
        str = str.replaceAll("\\)", " )");
        ...
        for (int i = 0; i < strArray.length; i++) {
            ...
            if (operations.contains(now)) {
                if ((i == 0 || i == strArray.length - 1)) {
                    throw new CustomException("Неверный формат ввода (начинается или заканичивается операцией))");
                }
                if (!lastNum) {
                    throw new CustomException("Неверный формат ввода (две операции подряд или ошибка у скобок)");
                }
                lastNum = false;
            }
            else if (now.equals("(")) {
                ...
                openBrackets -= 1;
                if (openBrackets < 0) {
                    throw new CustomException("Неверный формат ввода (неправильный порядок скобок)");
                }
            }
            else if (now.matches("-?\\d+")) {
                if (lastNum) {
                    throw new CustomException("Неверный формат ввода (два числа подряд или ошибка у скобок)");
                }
                lastNum = true;
            }
            else {
                throw new CustomException("Неверный формат ввода (встречается неизвестная подстрока)");
            }
        }
        if (openBrackets != 0) {
            throw new CustomException("Неверный формат ввода (неправильный порядок скобок)");
            ...
            switch (strArray[1]) {
                ...
                case "/" -> {
                    try {
                        ...
                        throw new CustomException("Ошибка: В строке происходит деление на ноль");
                    }
                }
            }
        }
